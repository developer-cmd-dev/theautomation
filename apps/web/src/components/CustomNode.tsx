...
function CustomNode({ data }: NodeProps<CustomNodeType>) {
  ...
  return (
    <div onDoubleClick={()=>setNodeDrawerHandleData({isCreated:true,nodeType:data.type})} title={data.title} className="bg-white border border-gray-600 p-2 rounded-md w-10 h-10 flex items-center justify-center aspect-video ">
      {data.icon}
